...
def message_tests(taxii_message, taxii_version='1.0'):
    """
    xml_binding is used to specify the XML Message Binding Spec version ID
    json_binding is used to specify the JSON binding spec version ID
    svcs_version is used to specify the dict version ID
    """
    if taxii_version == '1.0':
        xml_binding=t.VID_TAXII_XML_10
        json_binding=t.VID_CERT_EU_JSON_10
        svcs_version=t.VID_TAXII_SERVICES_10
    elif taxii_version == '1.1':
        xml_binding=t.VID_TAXII_XML_11
        json_binding=t.VID_CERT_EU_JSON_11
        svcs_version=t.VID_TAXII_SERVICES_11
    else:
        raise Exception('Unknown version of TAXII: %s', taxii_version)
    ...
    xml_string = taxii_message.to_xml(xml_binding)
    valid = tm.validate_xml(xml_string, xml_binding)
    if not valid:
        raise Exception('\tFailure of test #1 - XML not schema valid')
    msg_from_xml = tm.get_message_from_xml(xml_string, xml_binding)
    dictionary = taxii_message.to_dict(svcs_version)
    msg_from_dict = tm.get_message_from_dict(dictionary, svcs_version)
...
def contentblock_tests(content_block, taxii_version='1.0'):
    ...
    xml_string = content_block.to_xml(xml_binding)
    block_from_xml = tm.ContentBlock.from_xml(xml_string, xml_binding)
    dictionary = content_block.to_dict(svcs_version)
    block_from_dict = tm.ContentBlock.from_dict(dictionary, svcs_version)
    json_string = content_block.to_json(json_binding)
    block_from_json = tm.ContentBlock.from_json(json_string, json_binding)
...
# END TAXII 1.0 TESTS. THESE SHOULD NOT CHANGE

#BEGIN TAXII 1.1 TESTS
...
message_tests(discovery_request1, '1.1')
...
message_tests(poll_request1, '1.1')
...
message_tests(poll_response1, '1.1')
...
message_tests(poll_response2, '1.1')
...
message_tests(status_message1, '1.1')
...
message_tests(inbox_message1, '1.1')
...
message_tests(inbox_message2, '1.1')
...
message_tests(manage_feed_subscription_response1, '1.1')
